...
<div class="wrap crpcrm-admin-wrap crpcrm-requests-admin" dir="rtl">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="crpcrm-card crpcrm-manual-request-form">
		...
		<?php else : ?>
			...
			<?php foreach ( $selected_form['fields'] as $field ) : $field_name = sanitize_key( $field['name'] ); $field_type = sanitize_key( $field['type'] ?? 'text' ); $options = CRPCRM_Request_Forms::get_field_options( $field ); ?>
				<label class="<?php echo 'textarea' === $field_type ? 'crpcrm-full-field' : ''; ?>"><?php echo esc_html( $field['label'] ); ?><?php if ( 'textarea' === $field_type ) : ?><textarea name="<?php echo esc_attr( $field_name ); ?>" <?php echo ! empty( $field['required'] ) ? 'required' : ''; ?>></textarea><?php elseif ( in_array( $field_type, array( 'select', 'radio' ), true ) ) : ?><select name="<?php echo esc_attr( $field_name ); ?>" <?php echo ! empty( $field['required'] ) ? 'required' : ''; ?>><option value=""><?php echo esc_html( 'انتخاب کنید' ); ?></option><?php foreach ( $options as $value => $label ) : ?><option value="<?php echo esc_attr( $value ); ?>"><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select><?php else : ?><input type="<?php echo esc_attr( in_array( $field_type, array( 'email', 'number', 'date' ), true ) ? $field_type : 'text' ); ?>" name="<?php echo esc_attr( $field_name ); ?>" <?php echo ! empty( $field['required'] ) ? 'required' : ''; ?>><?php endif; ?></label>
			<?php endforeach; ?>
