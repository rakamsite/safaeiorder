...
<?php if ( 'daily_reports' === $tab ) : ?>
	...
	<?php foreach ( $items as $item ) : $snapshot = $get_snapshot( $item ); ?>
		<tr>
			<td><?php echo esc_html( CRPCRM_Helpers::format_jalali_date( $item['report_date'] ) ); ?></td>
			...
			<td>
				...
				<?php if ( $can_manage ) : ?>
					<div class="crpcrm-card"><h3>آمار CRM ثبت‌شده در زمان ارسال گزارش</h3>
						<?php if ( $snapshot ) : ?>
							<table class="widefat striped"><tbody><?php foreach ( $sales_snapshot_keys as $key ) : ?><tr><th><?php echo esc_html( $sales_stat_labels[ $key ] ?? $key ); ?></th><td><?php echo esc_html( 'generated_at' === $key ? CRPCRM_Helpers::format_jalali_datetime( $snapshot[ $key ] ) : absint( $snapshot[ $key ] ?? 0 ) ); ?></td></tr><?php endforeach; ?></tbody></table>
						<?php else : ?>
