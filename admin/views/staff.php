...
$employee_task_statuses = array_diff_key( $task_statuses, array( 'cancelled' => true ) );
...
if ( isset( $notice_labels['attachment_upload_failed'] ) ) {
	$notice_labels['attachment_finalize_failed'] = 'فایل موقت معتبر نیست یا منقضی شده است. لطفاً دوباره فایل را بارگذاری کنید.';
	$notice_labels['attachment_save_partial']   = 'متن ذخیره شد اما ذخیره فایل انجام نشد. لطفاً دوباره بررسی کنید.';
}
...
$user_label = function( $id ) { $user = get_userdata( absint( $id ) ); return $user ? $user->display_name : 'کاربر حذف‌شده'; };
$badge = function( $value, $labels ) { return '<span class="crpcrm-badge crpcrm-status-badge crpcrm-status-' . esc_attr( $value ) . '">' . esc_html( $labels[ $value ] ?? $value ) . '</span>'; };
$detail_url = function( $item_id ) use ( $tab ) { return add_query_arg( array( 'page' => 'crpcrm-staff', 'staff_tab' => $tab, 'staff_item_id' => absint( $item_id ) ), admin_url( 'admin.php' ) ); };
$list_url = add_query_arg( array( 'page' => 'crpcrm-staff', 'staff_tab' => $tab ), admin_url( 'admin.php' ) );
$announcement_data = $detail_item && 'announcements' === $tab ? CRPCRM_Helpers::maybe_json_decode( $detail_item['audience_data'], true ) : array();
?>
...
<div class="crpcrm-kpi-grid crpcrm-staff-kpi-grid">
<div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['open_tasks'] ); ?></span><span class="crpcrm-stat-label">وظایف باز من</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['overdue_tasks'] ); ?></span><span class="crpcrm-stat-label">وظایف عقب‌افتاده من</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['unread_announcements'] ); ?></span><span class="crpcrm-stat-label">اطلاعیه‌های خوانده‌نشده</span></div>
<?php if ( ! $can_manage ) : ?><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['today_report'] ); ?></span><span class="crpcrm-stat-label">گزارش امروز من</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['open_requests'] ); ?></span><span class="crpcrm-stat-label">درخواست‌های باز من</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['customer_replies'] ?? 0 ); ?></span><span class="crpcrm-stat-label">پاسخ جدید مشتری</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['lead_follow_ups'] ?? 0 ); ?></span><span class="crpcrm-stat-label">پیگیری خودکار ۲۴ ساعته</span></div><?php endif; ?>
</div>
<?php if ( $can_manage ) : ?><div class="crpcrm-kpi-grid crpcrm-staff-kpi-grid"><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( count( $dashboard_counts['reported_users'] ) ); ?></span><span class="crpcrm-stat-label">گزارش‌های امروز</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['attention_reports'] ); ?></span><span class="crpcrm-stat-label">گزارش نیازمند توجه</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['new_requests'] ); ?></span><span class="crpcrm-stat-label">درخواست جدید</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['unassigned_requests'] ?? 0 ); ?></span><span class="crpcrm-stat-label">درخواست‌های بدون مسئول</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['customer_replies'] ?? 0 ); ?></span><span class="crpcrm-stat-label">پاسخ جدید مشتری</span></div><div class="crpcrm-stat-card"><span class="crpcrm-stat-value"><?php echo esc_html( $dashboard_counts['lead_follow_ups'] ?? 0 ); ?></span><span class="crpcrm-stat-label">پیگیری خودکار ۲۴ ساعته</span></div></div>
<div class="crpcrm-card"><h2>خلاصه فروش امروز</h2><div class="crpcrm-table-wrap crpcrm-table-scroll"><table class="widefat striped"><thead><tr><th>کارشناس</th><th>گزارش</th><th>اقدامات</th><th>تماس پاسخ داده</th><th>پیگیری عقب‌افتاده</th><th>موفق</th><th>ناموفق</th></tr></thead><tbody><?php foreach ( $dashboard_counts['sales_today_summary'] as $row ) : $stats = $row['stats']; ?><tr><td><?php echo esc_html( $row['display_name'] ); ?></td><td><?php echo esc_html( $row['reported_today'] ? 'ثبت شده' : 'ثبت نشده' ); ?></td><td><?php echo esc_html( absint( $stats['actions_today'] ?? 0 ) ); ?></td><td><?php echo esc_html( absint( $stats['call_answered_today'] ?? 0 ) ); ?></td><td><?php echo esc_html( absint( $stats['overdue_followups'] ?? 0 ) ); ?></td><td><?php echo esc_html( absint( $stats['won_today'] ?? 0 ) ); ?></td><td><?php echo esc_html( absint( $stats['lost_today'] ?? 0 ) ); ?></td></tr><?php endforeach; ?></tbody></table></div></div><?php endif; ?>
<?php endif; ?>

<?php if ( $detail_item ) : ?>
<div class="crpcrm-card crpcrm-staff-detail"><div class="crpcrm-detail-heading"><h2>مشاهده جزئیات</h2><a class="button" href="<?php echo esc_url( $list_url ); ?>">بازگشت به فهرست</a></div>
...
<dt>کارمند</dt><dd><?php echo esc_html( $user_label( $detail_item['user_id'] ) ); ?></dd>
<dt>عنوان</dt><dd><?php echo esc_html( $detail_item['title'] ); ?></dd>
<dt>دسته‌بندی</dt><dd><?php echo esc_html( $categories[ $detail_item['category'] ] ?? $detail_item['category'] ); ?></dd>
<dt>شرح کامل</dt><dd><?php echo nl2br( esc_html( $detail_item['description'] ) ); ?></dd>
<?php if ( $request_attachment_html ) : ?><dt>پیوست فایل</dt><dd><?php echo $request_attachment_html; ?></dd><?php endif; ?>
...
<?php elseif ( 'tasks' === $tab ) : ?><th>عنوان</th><th>مسئول</th><th>مهلت</th><th>اولویت</th><th>وضعیت</th>
<?php else : ?><th>عنوان</th><?php if ( $can_manage ) : ?><th>مخاطب</th><th>ایجادکننده</th><th>تعداد مشاهده</th><?php endif; ?><th>تاریخ ایجاد</th><?php endif; ?><th>عملیات</th></tr></thead><tbody>
<?php foreach ( $items as $item ) : ?><tr>
<?php if ( 'daily_reports' === $tab ) : ?><td><?php echo esc_html( CRPCRM_Helpers::format_jalali_date( $item['report_date'] ) ); ?></td><td><?php echo esc_html( $user_label( $item['user_id'] ) ); ?></td><td><?php echo $badge( $item['status'], $report_statuses ); ?></td><td><?php echo esc_html( $item['needs_manager_attention'] ? 'بله' : 'خیر' ); ?></td>
<?php elseif ( 'requests' === $tab ) : ?><td><strong><?php echo esc_html( $item['title'] ); ?></strong></td><td><?php echo esc_html( $user_label( $item['user_id'] ) ); ?></td><td><?php echo esc_html( $categories[ $item['category'] ] ?? $item['category'] ); ?></td><td><?php echo $badge( $item['priority'], $priorities ); ?></td><td><?php echo $badge( $item['status'], $request_statuses ); ?></td>
<?php elseif ( 'issues' === $tab ) : ?><td><strong><?php echo esc_html( $item['title'] ); ?></strong></td><td><?php echo esc_html( $user_label( $item['user_id'] ) ); ?></td><td><?php echo esc_html( $item['related_department'] ); ?></td><td><?php echo $badge( $item['severity'], $severities ); ?></td><td><?php echo $badge( $item['status'], $issue_statuses ); ?></td>
<?php elseif ( 'tasks' === $tab ) : ?><td><strong><?php echo esc_html( $item['title'] ); ?></strong></td><td><?php echo esc_html( $user_label( $item['assigned_to'] ) ); ?></td><td><?php echo esc_html( CRPCRM_Helpers::format_jalali_date( $item['due_date'] ) ); ?></td><td><?php echo $badge( $item['priority'], $priorities ); ?></td><td><?php echo $badge( $item['status'], $task_statuses ); ?></td>
<?php else : $stats = $can_manage ? $repository->get_announcement_read_stats( $item['id'] ) : array(); ?><td><strong><?php echo esc_html( $item['title'] ); ?></strong></td><?php if ( $can_manage ) : ?><td><?php echo esc_html( $audiences[ $item['audience_type'] ] ?? $item['audience_type'] ); ?></td><td><?php echo esc_html( $user_label( $item['created_by'] ) ); ?></td><td><?php echo esc_html( $stats['seen'] . ' / ' . ( $stats['seen'] + $stats['unseen'] ) ); ?></td><?php endif; ?><td><?php echo esc_html( CRPCRM_Helpers::format_jalali_datetime( $item['created_at'] ) ); ?></td><?php endif; ?>
<td><a class="button" href="<?php echo esc_url( $detail_url( $item['id'] ) ); ?>">مشاهده جزئیات</a></td></tr><?php endforeach; ?><?php if ( empty( $items ) ) : ?><tr><td colspan="8">موردی یافت نشد.</td></tr><?php endif; ?></tbody></table></div>
<?php endif; ?>
</div>
