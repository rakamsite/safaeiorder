...
- هر فرم `form_id` مستقل دارد و می‌تواند `request_type` جدا داشته باشد.
- اگر `request_type` خالی باشد، runtime باید به `form_id` fallback کند.
- تکراری بودن `request_type` مجاز است، وقتی چند فرم باید زیر یک نوع درخواست مشترک گروه‌بندی شوند.
...
- `request_type` باید همیشه مقدار نهایی معتبر داشته باشد؛ اگر خالی باشد، باید به `form_id` fallback شود.
