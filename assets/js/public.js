(function () {
	...
	function renderPicker(input, year, month) {
		...
		next.addEventListener('click', function () {
			renderPicker(input, month === 12 ? year + 1 : year, month === 12 ? 1 : month + 1);
		});
		prev.addEventListener('click', function () {
			renderPicker(input, month === 1 ? year - 1 : year, month === 1 ? 12 : month - 1);
		});
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		document.querySelectorAll('.crpcrm-request-form').forEach(bindSubmitGuard);
